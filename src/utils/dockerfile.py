...
class DockerFile:  # pylint: disable=too-few-public-methods
    """
    DockerFile generator class
    """
    ...
    def _add_pip_requirements(self) -> None:
        ...
        addons_req_exists = os.path.exists(os.path.join(addons_path, 'requirements.txt'))

        if not odoo_path:
            return

        self._al(f'COPY {odoo_path}/requirements.txt /tmp/odoo_requirements.txt', 0)

        if addons_req_exists:
            addons_path = addons_path.replace(project_path, '.')
            self._al('COPY {addons_path}/requirements.txt /tmp/addons_requirements.txt', 0)
    ...
    def _add_init_scripts(self):
        self._al('COPY --chown=odoo:odoo ./docker/wait-for-psql.py '
                 '/usr/local/bin/wait-for-psql.py', 0)
        self._al('COPY --chown=odoo:odoo ./docker/entrypoint.sh /entrypoint.sh', 0)
        self._add_spacer()
